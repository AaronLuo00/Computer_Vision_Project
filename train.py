...
if args.model == '2_generator_base_fml_loss':
    config.GENERATOR_TYPE = 'UNet'
    config.LOSS_TYPE = 'fml'
    
if args.model == '3_generator_base_l1_and_fml_loss':
    config.GENERATOR_TYPE = 'UNet'
...
additional_criterion = torch.nn.L1Loss() if config.LOSS_TYPE == 'l1' else FeatureMatchingLoss(color='rgb')
